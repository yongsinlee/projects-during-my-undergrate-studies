...
- 메시지 전송: ROS에서 계산된 명령을 MAVLink 패킷으로 캡슐화하여 기체에 전송

- GCS 연동: 미션 플래너(Mission Planner)를 통해 기체 상태(Heartbeat) 실시간 모니터링 및 파라미터 원격 조정 환경 구축
...
- 가변 속도 제어: 곡률 및 오차 크기에 따라 선속도를 실시간으로 조정하는 알고리즘 추가

## 6. 이외 추가 학습 내용
- 차세대 환경 확장: Raspberry Pi 5 및 ROS2 Humble 환경에서 LiDAR 센서를 연동하여 실시간 객체 인식 및 항법 시스템 구축 실습
  
- GPS의 작동 원리와 위성 신호 생성 및 CDMA 기술 기반의 위성 분류 체계를 학습
  
- 마스터-슬레이브 구조의 I2C 통신과 비동기 직렬 방식인 UART 통신의 프로토콜 특성 및 하드웨어 연결 방식에 대해 학습
