...
% Last Modified by GUIDE v2.5 03-Dec-2024 14:58:51
...
if length(user)>0 && length(user_pre)>0
    ...
    for i = 1:length(usernew)
        ...
        idindexx = find(strcmp({user.id},matchinguser.id));
        ...
        if strcmp(user_mypre.want, user_pre(idindex).want)
            ...
            if strcmp(user_mypre.gender,'all') && ...
               ...
               (strcmp(user_pre(idindex).gender, 'all') || strcmp(user_pre(idindex).gender,mine.gender)) && ...
               (strcmp(user_pre(idindex).major, 'all') || ...  % 상대의 전공 선호 무관
               strcmp(user_pre(idindex).major,'same') && strcmp(user(idindexx).major,mine.major) ||... % 상대의 전공 선호 동일
               strcmp(user_pre(idindex).major,'other') && ~strcmp(user(idindexx).major,mine.major)) && ... % 상대의 전공 선호 비동일
               (strcmp(user_pre(idindex).grade, 'all') || strcmp(user_pre(idindex).grade,mine.grade))
            ...
            elseif strcmp(user_mypre.gender, 'all') && ...
                ...
                % 상대의 선호가 전공 무관이거나 내 정보와 동일한 경우
                if strcmp(user_mypre.major,'all') && ...
                        (strcmp(user_pre(idindex).major, 'all') || ...  % 상대의 전공 선호 무관
                        strcmp(user_pre(idindex).major,'same') && strcmp(user(idindexx),mine.major) ||... % 상대의 전공 선호 동일
                        strcmp(user_pre(idindex).major,'other') && ~strcmp(user(idindexx),mine.major)) % 상대의 전공 선호 비동일
                    % 선호 학년과 상대 학년이 같은 경우
                    ...
                    % 상대의 선호가 학년 무관이거나 내 정보와 동일한 경우
                elseif  strcmp(user_mypre.grade,'all') && ...
                        (strcmp(user_pre(idindex).grade, 'all') || strcmp(user_pre(idindex).grade,mine.grade))
                    % 선호 전공과 상대 전공이 같은 경우
                    if (strcmp(user_mypre.major,'same') && strcmp(user(idindexx),mine.major) ||... % 나의 전공 선호 동일
                        strcmp(user_mypre.major,'other') && ~strcmp(user(idindexx),mine.major)) && ...
                       (strcmp(user_pre(idindex).major,'same') && strcmp(user(idindexx).major,mine.major) ||... % 상대의 전공 선호 동일
                        strcmp(user_pre(idindex).major,'other') && ~strcmp(user(idindexx).major,mine.major))
                        matcheduser(c).id = matchinguser.id;
                        matcheduser(c).gender = matchinguser.gender;
                        matcheduser(c).major = matchinguser.major;
                        matcheduser(c).grade = matchinguser.grade;
                    ...
                    % 내 학년, 전공 선호와 상대 학년, 전공이 같을 경우
                    if (strcmp(user_mypre.major,'same') && strcmp(user(idindexx).major,mine.major) ||... % 나의 전공 선호 동일
                        strcmp(user_mypre.major,'other') && ~strcmp(user(idindexx).major,mine.major)) && ...
                       (strcmp(user_pre(idindex).major,'same') && strcmp(user(idindex).major,mine.major) ||... % 상대의 전공 선호 동일
                        strcmp(user_pre(idindex).major,'other') && ~strcmp(user(idindex).major,mine.major)) && ...
            ...
            elseif strcmp(user_mypre.grade, 'all') && ...
                    (strcmp(user_pre(idindex).grade, 'all') || strcmp(user(idindexx).grade,mine.grade))

                % 내 선호가 학년 무관 + 전공 무관 + 성별 동일인 경우 위에서 카운트 완료
                % 내 선호가 학년 무관 + 성별 무관 + 전공 동일인 경우 위에서 카운트 완료
                % 내 선호가 학년 무관 + 성별 동일 + 전공 동일인 경우
                if  strcmp(user_mypre.gender, matchinguser.gender) && ...
                    (strcmp(user_mypre.major,'same') && strcmp(user(idindexx).major,mine.major) ||... % 나의 전공 선호 동일
                     strcmp(user_mypre.major,'other') && ~strcmp(user(idindexx).major,mine.major)) && ...
                    (strcmp(user_pre(idindex).major,'same') && strcmp(user(idindexx).major,mine.major) ||... % 상대의 전공 선호 동일
                     strcmp(user_pre(idindex).major,'other') && ~strcmp(user(idindexx).major,mine.major))
                    matcheduser(c).id = matchinguser.id;
    ...
    if length(pre_day)>0 && length(user_day)>0
        ...
        if strcmp(user_mypre.want,'meal')
            ...
            for p = 1:length(pre_day(:,1))
                ...
                if pre_day{p,2} == user_day{2} && pre_day{p,3} == 0 && user_day{3} == 0
                    ...
                    matched_places{a} = char(classname(user_day{2}));

                    a = a + 1;
                    % 3교시 끝나는 건물이 같고 4교시 수업이 없는 경우 (13시 30분 식사)
                elseif pre_day{p,3} == user_day{3} && pre_day{p,4} == 0 && user_day{4} == 0
                    matched(a).day = user_mypre.day;
                    matched(a).kakao = pre_day{p,8};
                    ...
                    matched(a).gender = pre_day{p,10};
                    matched(a).grade = pre_day{p,11};
                    matched_places{a} = char(classname(user_day{3}));
        ...
        elseif strcmp(user_mypre.want,'study') %끝나는 수업과 건물이 같은경우
            ...
            for p=1:length(pre_day(:,1))
                ...
                if user_last_class(1)==pre_last_class(1) && user_last_class(2)==pre_last_class(2)
                    matched(a).day = user_mypre.day;
                    matched(a).kakao = pre_day{p,8};
                    matched(a).time = idxxxx ;
                    matched(a).major = pre_day{p,9};
                    matched(a).gender = pre_day{p,10};
                    matched(a).grade = pre_day{p,11};
                    matched_places{a} = char(classname(user_last_class(2)));
            ...
            for i = 1:length(matched)
                ...
                for j = 1:length(matched(i).time)
                    if j == length(matched(i).time)
                        fprintf(fid5,'%d교시(%s)\n',matched(i).time(j),matched_places{1,j});
                    else
                        fprintf(fid5,'%d교시(%s)\t',matched(i).time(j),matched_places{1,j});
                    end
                end
            end
            fclose(fid5);
